...
- **`figure_options.py`** — matplotlib 내장 "Figure options"(Customize) 다이얼로그(`matplotlib.backends.qt_editor.figureoptions.figure_edit`)를 그대로 옮겨온 뒤 X/Y축 Scale(linear/log/symlog/logit) 드롭다운과 Title/축 Min·Max·Label/legend 재생성 체크박스를 담은 "Axes" 탭 전체를 뺀 버전입니다(`figure_edit()` — 남은 건 "Curves" 탭뿐). matplotlib이 이 다이얼로그를 필드 단위로 커스터마이즈하는 공개 API를 제공하지 않아서 함수 전체를 vendoring했으니, matplotlib을 업그레이드할 때는 이 파일을 원본과 다시 비교해서 다른 변경 사항이 있는지 확인해야 합니다. 축 Min/Max/Label 편집은 이 다이얼로그가 아니라 `axis_edit()`(그래프에서 축을 더블클릭하면 뜸)이 담당합니다 — Axes 탭은 항상 첫 번째 축(`figure_edit`의 `axes` 인자)만 편집할 수 있어서 twinx로 만든 추가 Y축을 건드릴 수 없었는데, `axis_edit()`은 더블클릭한 축 하나하나를 직접 받으므로 축이 몇 개든 상관없습니다. Curves 탭에서 라인 색을 바꾸면 그 라인이 속한 축의 y라벨/눈금 색도 같은 색으로 맞추고(`apply_callback`), `figure_edit(..., on_apply=...)`로 넘겨받은 콜백(`PlotCanvas.refresh_legend`)을 호출해 범례 아이콘도 새 색으로 다시 그립니다 — matplotlib 범례는 라인을 그대로 참조하지 않고 생성 시점 색의 복사본을 그리므로, 이렇게 안 하면 라인 색만 바뀌고 범례/축 색은 예전 색 그대로 남습니다. `main_window.py`의 `_PlotToolbar.edit_parameters()`가 matplotlib 기본 `figureoptions.figure_edit` 대신 이 모듈을 호출하도록 오버라이드합니다.
- **`plot_canvas.py`** — `PlotCanvas(FigureCanvasQTAgg)`는 matplotlib figure를 Qt에 임베드합니다. `plot_lines()`는 요청받은 각 Y 컬럼을 `pd.to_numeric(errors="coerce")`로 강제 변환하고, 전부 NaN이 되어버린 컬럼은 예외를 던지지 않고 건너뛴 뒤 호출자에게 돌려줍니다(어떤 컬럼이 제외됐는지 UI가 알릴 수 있도록). X축이 숫자/날짜형이 아니면(예: 타임스탬프 문자열) 위치 인덱스를 실제 플롯 좌표로 쓰고 `FuncFormatter`로 눈금에만 원래 문자열을 붙입니다 — matplotlib이 문자열 X축을 범주형으로 취급해 값마다 변환하면 수십만~수백만 행에서 렌더링이 몇 분씩 걸리기 때문입니다(실측: 128만 행 기준 범주형 축 그대로면 15분 넘게 안 끝남, 위치 인덱스로 우회하면 4초). 마커는 행이 500개 이하일 때만 그리고(대용량에서는 점이 뭉개져 안 보이는 데다 렌더링만 느려짐), 범례는 자동 위치 탐색(`loc="best"`) 대신 `"upper right"`로 고정합니다 — `best`는 겹침 회피를 위해 모든 데이터 포인트를 훑어서 대용량에서 그 자체로 느립니다. 모듈 최상단의 `plt.rcParams["font.family"] = "Malgun Gothic"` 설정은 한글 라벨이 깨진 네모(□)로 나오지 않게 하기 위한 것이니, 다른 한중일 폰트로 대체하지 않는 한 제거하면 안 됩니다. `__init__`에서 `scroll_event`를 `_on_scroll`에 연결해 마우스 커서 위치를 중심으로 휠 확대/축소를 지원하고, `button_press_event`를 `_on_axis_double_click`에 연결해 축(눈금+라벨)을 더블클릭하면 `figure_options.axis_edit()`으로 그 축 하나의 Min/Max/Label을 바로 편집할 수 있게 합니다 — Axis(XAxis/YAxis)는 matplotlib Artist의 pick 가능한 `contains()`를 구현하지 않아서(기본 구현은 항상 False), `get_tightbbox()`가 돌려주는 디스플레이 좌표 bbox로 직접 히트 테스트합니다. 범례는 `_rebuild_legend()`(handles/labels로 그리고 드래그 위치를 유지)로 만들고, `refresh_legend()`는 지금 그려진 라인들의 handle/label로 이걸 다시 호출합니다 — Customize에서 라인 색을 바꾼 뒤 범례 아이콘을 최신 색으로 갱신하는 데 씁니다(자세한 이유는 `figure_options.py` 설명 참고).
- **`main_window.py`** — 모든 UI 상태와 흐름을 담당합니다. 눈에 잘 안 띄는 핵심 동작:
  ...
  - 그래프 위에는 `_PlotToolbar`(모듈 상단에 정의된 `NavigationToolbar2QT` 서브클래스, `self._plot_toolbar`)를 붙여 부분 확대/이동/원래대로/Customize(곡선 편집)/저장을 제공합니다. 기본 `toolitems`에서 **Back/Forward(이전·다음 보기)만 뺐습니다**: `_update_plot`이 매번 `self._plot_toolbar.update()`로 뷰 기록을 통째로 지우기 때문에(아래 항목 참고) 애초에 되돌아갈 기록이 안 남아 실사용상 무의미해서입니다. "Subplots"(Configure subplots) 버튼은 그대로 뒀지만, Wspace/Hspace(서브플롯 사이 간격) 슬라이더는 이 앱에서 아무 효과가 없습니다 — `add_subplot(111)`로 서브플롯이 항상 1개뿐이고, Y축이 여러 개여도 `twinx()`로 같은 서브플롯 자리에 겹쳐 그리는 것이지 그리드에 서브플롯이 늘어나는 게 아니라서 "다른 서브플롯과의 간격"이라는 개념 자체가 성립하지 않기 때문입니다(Left/Right/Bottom/Top 여백은 동작하지만 `plot_lines()`가 다시 그릴 때마다 `subplots_adjust()`로 자체 계산한 값으로 덮어써서 오래 유지되지 않습니다). `_PlotToolbar.edit_parameters()`를 오버라이드해서 Customize 버튼이 matplotlib 기본 다이얼로그 대신 `app/figure_options.py`(Scale 드롭다운과 Axes 탭을 뺀 버전, Curves 탭만 남음)를 열도록 바꿨습니다 — 이 앱은 장비 로그 시계열만 다루고 로그/symlog 축이 쓸모없다는 사용자 요청, 그리고 축 편집은 더블클릭(아래 `plot_canvas.py` 항목의 `axis_edit` 참고)으로 옮겨갔다는 이유에 따른 것입니다.
